	...
	class Frontend:
		...
		def create_handicap_display(self) -> toga.Box:
			...
			handicap_label = toga.Label(
				f"Handicap Index: {self.backend.handicap_index}",
		...
		def display_table(self):
			...
			csv_file = self.backend.declare_csv_directory()
		...
		def save_score(self, widget) -> None:
			...
			if inputs_are_valid:
				...
				differential = self.backend.calculate_round_differential(adjusted_gross_score, rating, slope)
				# Write inputs to CSV
				self.backend.write_to_csv(course_name, date, adjusted_gross_score, rating, slope, differential)
